				   ...
				   unsigned int pb)
{
	...
	if (!virt) {
		v4l2_err(&dev->v4l2_dev,
			 "dma%d: unable to allocate %s-buffer [len: %u, w: %u, h: %u, d:%u]\n",
			 vc->ch, pb ? "B" : "P", len, vc->width, vc->height, vc->format->depth);
		return -ENOMEM;
	}
			       ...
			       unsigned int pb)
{
	...
	if (desc->size) {
		...
		if (!virt) {
			v4l2_err(&dev->v4l2_dev,
				 "dma%d: unable to allocate %s-buffer [size: %u]\n",
				 vc->ch, pb ? "B" : "P", desc->size);
			return -ENOMEM;
		}
		desc->virt = virt;
...
int tw686x_video_init(struct tw686x_dev *dev)
{
	...
	for (ch = 0; ch < max_channels(dev); ch++) {
		struct tw686x_video_channel *vc = &dev->video_channels[ch];
		dev_err(&dev->pci_dev->dev, "registor channel %u\n", ch);
		vc->dev = dev;
		vc->ch = ch;
	}
